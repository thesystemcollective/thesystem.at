export default () => {
  ...
  var imageLoaded = ({ canvas, image }) => {
    ...
    const imageData = createImageData({ ctx, image });
    const pixels = processImageData({ image, data: imageData.data });
    const onComplete = ({ canvas: canvas2, timeline: timeline3 }) => () => {
      animate = false;
      const onHover = () => {
        if (!animate) {
          animate = true;
          timeline3.restart();
          canvas2.removeEventListener("mouseover", onHover);
        }
      };
      canvas2.addEventListener("mouseover", onHover);
    };
    const timeline2 = gsapWithCSS.timeline({ repeat: 1, yoyo: true })
      .to(gvars, {
        duration: 4,
        waveAmp0: 64,
        waveFrq0: 16,
      ...
      .add(gvars, { duration: 1, pos0: 256, ease: Bounce.easeInOut, delay: 4 })
      .add(gvars, { duration: 5, pos1: 128, pos2: 512, ease: Cubic.easeInOut, delay: 2 });

    timeline2.eventCallback("onComplete", onComplete({ canvas, timeline: timeline2 }));
    const html = document.querySelector("html");
    const offset = image.offsetTop + image.height;
    timeline2.paused(offset < html.scrollTop);
    requestAnimationFrame(() => loop({ ctx, image, imageData, pixels }));
    window.addEventListener("scroll", () => {
      if (!timeline2._ps && offset < html.scrollTop) {
        timeline2.paused(true);
        animate = false;
      } else if (offset > html.scrollTop) {
        if (timeline2._ps) {
  ...
  var processImageData = ({ image, data }) => {
  ...
  var update = ({ image, imageData, pixels }) => {
    cosCache0.length = 0;
    cosCache1.length = 0;
    for (var y = 0; y < image.height; y++) {
      const fy = y / image.width;
      const wave0Sin = Math.sin(gvars.waveOfs0 + gvars.waveFrq0 * fy) * gvars.waveAmp0;
      const wave1Sin = Math.sin(gvars.waveOfs1 + gvars.waveFrq1 * fy) * gvars.waveAmp1;
      const pos1 = gvars.pos1 * fy;
      for (var x = 0; x < image.width; x++) {
        const pixelIndex = (y * image.width + x) * 4;
        const fx = x / image.width;
        let wave0Cos;
        if (cosCache0[x] !== void 0) {
        ...
        let wave1Cos;
        if (cosCache1[x] !== void 0) {
          wave1Cos = cosCache1[x];
        } else {
          wave1Cos = Math.cos(gvars.waveOfs1 + gvars.waveFrq1 * fx) * gvars.waveAmp1;
          cosCache1[x] = wave1Cos;
        }
        const pos0 = gvars.pos0 * fx;
        const rx = x - wave0Sin - pos0;
        const ry = y - wave0Sin + pos1;
        const gx = x - wave1Sin + pos0;
        const gy = y - wave0Cos + pos1;
        const bx = x - wave1Sin + pos0;
        const by = y - wave0Cos + pos1;
        imageData.data[pixelIndex + 0] = getChannel(image, pixels, rx, ry, 0);
        imageData.data[pixelIndex + 1] = getChannel(image, pixels, gx, gy, 1);
        imageData.data[pixelIndex + 2] = getChannel(image, pixels, bx, by, 2);
      }
    }
    return imageData;
  };
  var loop = (props) => {
    if (animate) {
      const { ctx } = props;
      props.imageData = update(props);
      ctx.putImageData(props.imageData, 0, 0);
    }
    requestAnimationFrame(() => loop(props));
  };
  function getChannel(image, pixels, x, y, c) {
    x = x | 0;
    y = y | 0;
    x = wrap3(x, 0, image.width - 1);
    y = wrap3(y, 0, image.height - 1);
    return pixels[y][x][c];
  }
  function wrap3(v, min, max) {
    return ((v - min) % (max - min) + (max - min)) % (max - min) + min;
  }

  const image = document.querySelector(".HeroLogo");
